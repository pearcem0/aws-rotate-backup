...
configlocation="$HOME/.aws/config"
...
do
    ...
    read -t 300 -p "Going to sleep to make sure instance gets created before trying to fetch name. To skip press ENTER" 
    NewImageNames+=(${BackupInstanceNAMES[$NameCounter]}_BACKUP_$now)

    ((NameCounter++))
    echo "\nDone."
done

echo "added new image IDs to array"
for i in "${NewImageNames[@]}"
do
    echo "Image Name: $i"
    echo "Image ID: "
    aws ec2 describe-images --profile $chosenprofile --output table --filters 'Name=name,Values='$i'' |grep 'ami-'|awk '{ print $4 ; }'

    NewImageIDs+=($(aws ec2 describe-images --profile $chosenprofile --output table --filters 'Name=name,Values='$i'' |grep 'ami-'|awk '{ print $4 ; }'))
done
echo "printing NewImageIDs array (debug)"
printf '%s\n' "${NewImageIDs[@]}"

echo "Tagging new images as rotation 1.."

for i in "${NewImageIDs[@]}"
do
    echo "AMI ID: $i"
    aws ec2 create-tags --profile $chosenprofile --resources $i --tag Key=Rotation,Value=1
    echo "Done."
done
